...
public:
    ...
    char* decompress(unsigned char compressedChar)//unsigned char compressedChar
    {
        //char temy[]= " ";
       // temy[0] = compressedChar;
        for (int i = 0; i < count; i++)
        {
            ...
            if (compressedChar == '*'|| compressedChar == ' ')
            {
                char temp[] = "forrest";
                return temp;
            }*/
    ...
    void startComp()
    {
        ...
        if (obj != NULL)
        {
            while (fscanf(obj, "%s", word) != EOF)
            {
                ...
                /*if (0 == strcmp("said", word))///////dictionary[count].character = count + 130;
                {   

                    
                    strcpy(temp, strcat(temp,word));
                    //strcat(temp,(char*)compress(word));
                    strcat(temp, " ");
                    //fprintf(out_file, " ");
                    printf("\ncheck3");
                    }*/
                 if (0 == strcmp("Momma", word))
                {
                    strcpy(temp, strcat(temp, word));
                    //strcat(temp,(char*)compress(word));
                    strcat(temp, " ");
    ...
    void startDecompress()
    {
        ...
        {
            for (int i = 0; i < 5500; i++)//loop size must be equal or less than the count of words in i/p text i:e for r i/p fileb 4 or less
            {
